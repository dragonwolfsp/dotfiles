...
class MelodyneEditFilter(MelodyneEditText):

	def _isEditing(self):
		box = self.box.getBox()
		i = FindNearestColor(box.hwnd, box.left + 10, box.top - 2, (0xc1c1c1, 0xdcc1a9))
		return i == 1

	def focusLost(self):
		if self._isEditing():
			box = self.box.getBox()
			outside = Box(box.hwnd, box.left - 10, box.top)
			outside.leftClick()

	def getTextInfo(self):
		type, name, text = super(MelodyneEditFilter, self).getTextInfo()
		if text == "Q":
			text = "Not set"
...
class MelodyneAmplitudeTab(MelodyneSimpleTab):

	def __init__(self, name, sibi, pt, black_pt1, getShift = None):
		super(MelodyneAmplitudeTab,self).__init__(name, sibi, pt)
		self.black_1 = Box(sibi.hwnd, sibi.xScale(black_pt1), sibi.yScale(black_pt1), getShift)
			
	def isActive(self):
		if not super(MelodyneAmplitudeTab,self).isActive():
			return False
		box = self.black_1.getBox()
		return FindNearestColor(box.hwnd, box.left, box.top, (0xfdfdfd, 0x000000)) == 1

class MelodyneSibilantBalanceTab(MelodyneAmplitudeTab):
...
class Melodyne(SIBINVDA):
	...
	def isEssencial(self):
		...
		# log.error("Detected Melodyne type: '%d'" %(i))		
		return i == 0
		
	def hasSibilantBalance(self):
		pt = Pt(161, 76)
		x = self.sibi.xScale(pt)
		y = self.sibi.yScale(pt)
		i = FindNearestColor(self.sibi.hwnd, x, y, ( 0x000000, 0xbcbcbc, 0xf3f3f3))
		return i == 0
	...
	def defineSIBI(self):
		...
		if not essential :
			sibilant = self.hasSibilantBalance()
			...
			if sibilant:
				tools.add( MelodyneAmplitudeTab("Amplitude", self.sibi, Pt(153, 60), Pt(153, 67)) )
			else:
			...
			tools.add( MelodynePitchDriftAttackTab("Attack speed", self.sibi, Pt(175, 60), Pt(177, 62), Pt(177, 72)) )		
			if sibilant:
				tools.add( MelodyneSibilantBalanceTab("Sibilant balance", self.sibi, Pt(153, 60), Pt(151, 67)) )
		self.sibi.add( tools )
...
class MelodynePopupMenu(Label):
	"""
	It display GNWindowMenu, but it does not move system focus there. The position is dynamic
	Similar to MelodyneCombo, except there is no selection and it is always enabled
	"""
	def __init__(self, name, sibi, box_left_top, box_right_bottom, list_active_color, list_inactive_color, opt = None, getShift = None ):
		super(MelodynePopupMenu,self).__init__(name, sibi, box_left_top, box_right_bottom, opt, getShift)
		# self.type = "Menu"
		self.list_active_colors = Color2Tuple(list_active_color)
		self.list_inactive_colors = Color2Tuple(list_inactive_color)
		self.list_colors = self.list_active_colors + self.list_inactive_colors
		self.list_pt = XY((self.box.left + self.box.right)//2, self.box.bottom + 10)
		self.list_hwnd = None
		self.getShift = getShift
		self.list_item = TextBox(self.sibi.hwnd, 0, 0) # hash holder, real one will be self.list_hwnd with dynamic coordinates
		if getShift is None:
	...
	def getTextInfo(self):
		...
		if shift is None:
			return (self.type, self.name, "disabled")
		if not self._isOpen(shift):
			return super(MelodynePopupMenu,self).getTextInfo()
		text = self._getListItemText()
		if text == '':
			text = "not selected"
		return (self.type, "list item", text)
...
class MelodyneRadioBtns(Control):
	def __init__(self, name, sibi, btns_pt, btns_labels, colors, help = ''):
		super(MelodyneRadioBtns,self).__init__(name, sibi, None)
		self._btns = []
...
class MelodyneQuantizeTime(SIBINVDA):
	...
	def defineSIBI(self):
		self.sibi = SIBI(self.windowHandle, self.location.width, self.location.height, None)
		self.sibi.add( MelodyneRadioBtns("Groove reference", self.sibi,
										 (Pt(19,40), Pt(65,40), Pt(113,40), Pt(160,40), Pt(210,40), Pt(255,40), Pt(307,40), Pt(352,40), Pt(401,40)),
										 ("1/4", "1/4 T", "1/8", "1/8 T", "1/16", "1/16 T", "1/32", "1/32 T", "Auto"),
										 (0xcecece, 0xf9f9f9) ) )
		self.sibi.add( Fader("Intensity", self.sibi, Pt(412, 75), Pt(452, 97), Pt(255, 85), ("dynamic_text", )) )		
		self.sibi.add( CheckBtn("Include notes edited manually", self.sibi, Pt(18, 117), Pt(18, 117), Pt(18, 117), 0xbdbdbd, 0xcacaca, 0x0e0e0e, None) )
		self.sibi.add( Clickable("Ok", self.sibi, Pt(454, 117), Pt(454, 117), ("silent_action",)) )
		self.sibi.cancel = Clickable("Cancel", self.sibi, Pt(363, 116), Pt(363, 116), ("silent_action",))
		self.sibi.add( self.sibi.cancel )
...
class MelodyneNoteLeveling(SIBINVDA):
	...
	def defineSIBI(self):
		self.sibi = SIBI(self.windowHandle, self.location.width, self.location.height, None)
		self.sibi.add( Fader("Make quiet notes louder", self.sibi, Pt(82, 51), Pt(130, 68), Pt(125, 40), ("dynamic_text", )) )
		self.sibi.add( Fader("Make loud notes quieter", self.sibi, Pt(291, 51), Pt(352, 68), Pt(340, 40), ("dynamic_text", )) )
		self.sibi.add( Clickable("Ok", self.sibi, Pt(425, 86), Pt(425, 86), ("silent_action",)) )
		self.sibi.cancel = Clickable("Cancel", self.sibi, Pt(334, 86), Pt(334, 86), ("silent_action",))
		self.sibi.add( self.sibi.cancel )
...
class VListOfBoxes(Control):
	...
	def _getRows(self):
		...
		while y < box.bottom:
			...
			if height < 5:
				# log.info("Too thin row: %d %d", top, height)
				break
			y = top + height # so, bg line after
			rows.append(Box(box.hwnd, box.left, top - 1, box.right, y))
			# log.info("Row: %d %d", top, height)
		# log.info("No more rows till %d", box.bottom)
		return rows
...
class MelodyneShortcuts_v5(VListOfBoxes):
	def __init__(self, name, sibi, left_top, right_bottom):
		list_rb = Pt(right_bottom.x - 230, right_bottom.y)
		# log.error("%d %d" % (list_rb.x, list_rb.y))
		super(MelodyneShortcuts_v5,self).__init__(name, sibi, left_top, list_rb, (0xbfbfbf, 0xf9f9f9), (0x000000, 0x686868),
			Pt(right_bottom.x, left_top.y + 11), Pt(right_bottom.x, right_bottom.y - 4), 0xbfbfbf, 0x6c6c6c)
		self.type = "Treeview"
		
		self.item_open = sibi.TextBox(Pt(6,0), Pt(20,0))
		self.item_action = sibi.TextBox(Pt(21, 0), Pt(right_bottom.x - 143, 0))
		self.item_shortcut = sibi.TextBox(Pt(right_bottom.x - 136, 0), Pt(right_bottom.x - 29, 0))
		self.item_delete = sibi.Box(Pt(right_bottom.x - 20, 0))
		
		self._initial_idx_done = False
		# self.__initial_idx() postponed till the first access (the page can be invisible at initialization)
		
	def __initial_idx(self):
		if self._initial_idx_done:
			return
		self._initial_idx_done = True
		box = self.list.getBox()
		top, bottom = FindVRange(box.hwnd, box.left, box.top, box.bottom, 0xbfbfbf, 0xf9f9f9)
		if top < 0:
			return
	...
	def __updateItemPosition(self, box):
		...
		self.item_action.top = box.top
		self.item_shortcut.top = box.top
		self.item_delete.top = box.top
		self.item_open.bottom = box.bottom
		self.item_action.bottom = box.bottom
		self.item_shortcut.bottom = box.bottom
		self.item_delete.bottom = box.bottom
	...
	def onDelete(self):
		...
		if idx is None:
			return False
		self.__updateItemPosition(box)
		self.item_delete.leftClick()
		return True
			

class MelodynePreferences(SIBINVDA):
	knownPages = { "Shortcuts": 0, "Recording": 1, "Audio": 2, "Chedc for Updates": 3, "Check for Updates": 3, "User Interface": 4,
	...
	def _getShortcutsVersion(self):
		box = self.sibi.Box(Pt(self.location.width - 16, 60))
		i = FindNearestColor(box.hwnd, box.left, box.top, (0x0e0e0e, 0xbfbfbf))
		if i == 0 :
			return 1
		return 0;

	def _getUserInterfaceVersion(self):
		box = self.sibi.Box(Pt(155, 218))
		i = FindNearestColor(box.hwnd, box.left, box.top, (0x929292, 0xbfbfbf))
		if i == 0 :
			return 1
		return 0;

	def _getAudioVersion(self):
		box = self.sibi.Box(Pt(87, 197))
		i = FindNearestColor(box.hwnd, box.left, box.top, (0x000000, 0xbfbfbf))
		if i == 0 :
			return 1
		return 0;
	...
	def defineSIBI(self):
		self.sibi = SIBI(self.windowHandle, self.location.width, self.location.height, None)
		self.page_combo = MelodyneCombo("Page", self.sibi, Pt(12, 13), Pt(263, 27), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5)
		self.sibi.add( self.page_combo )
		getPageIdx = lambda : self._getPageIdx()
		pages = Pages("Preferences pages", self.sibi, getPageIdx)
		
		getShortcutsIdx = lambda : self._getShortcutsVersion()
		shortcuts = Pages("Shortcuts versions", self.sibi, getShortcutsIdx);
		...
		page = Container("Shortcuts", self.sibi, None)
		page.add( MelodyneEditFilter("Filter by name", self.sibi, Pt(13, 55), Pt(206, 65)) )
		page.add( MelodynePopupMenu("Menu", self.sibi, Pt(self.location.width - 21,57), Pt(self.location.width - 16, 62), 0x9c9c9c, 0xf5f5f5) )
		page.add( MelodyneShortcuts_v5("Shortcuts", self.sibi, Pt(7, 80), Pt(self.location.width - 9, self.location.height - 23)) )
		shortcuts.add(page)
		...
		page.add( MelodyneCombo("Audio file format", self.sibi, Pt(153, 96), Pt(284, 109), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )
		pages.add(page)

		getAudioIdx = lambda : self._getAudioVersion()
		audio = Pages("Audio", self.sibi, getAudioIdx);
		...
		# v5
		page = Container("Audio", self.sibi, None)
		page.add( MelodyneCombo("Audio device", self.sibi, Pt(152, 51), Pt(310, 65), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )
		page.add( MelodyneCombo("Sample rate", self.sibi, Pt(152, 74), Pt(355, 88), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )
		page.add( MelodyneCombo("Buffer size", self.sibi, Pt(152, 97), Pt(353, 111), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )
		page.add( MelodyneRadioBtn("Ignore buffer underruns", self.sibi, Pt(0,0), None, Pt(157, 126), 0xbfbfbf, 0xcacaca, 0x0e0e0e, None) )
		page.add( MelodyneCombo("Master output", self.sibi, Pt(152, 167), Pt(218, 181), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )
		page.add( MelodyneCombo("Default input", self.sibi, Pt(152, 190), Pt(217, 204), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )
		audio.add(page)
		
		pages.add(audio)

		page = Container("Updates", self.sibi, None)
		page.add( MelodyneCombo("Check for updates", self.sibi, Pt(152, 51), Pt(259, 64), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )
		...
		getUserInterfaceIdx = lambda : self._getUserInterfaceVersion()
		user_interface = Pages("UserInterface", self.sibi, getUserInterfaceIdx);
		...
		user_interface.add(page)

		# v5
		page = Container("User interface", self.sibi, None)
		page.add( MelodyneCombo("Language", self.sibi, Pt(153, 51), Pt(333, 65), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )
		page.add( MelodyneCombo("Pitch labels", self.sibi, Pt(153, 73), Pt(325, 87), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )

		page.add( MelodyneCombo("Look", self.sibi, Pt(152, 139), Pt(327, 153), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )
		page.add( MelodyneEditText("Default tuning", self.sibi, Pt(153, 163), Pt(222, 175)) )
		page.add( MelodyneCombo("Maximum undo levels", self.sibi, Pt(152, 183), Pt(208, 197), 0xc3c3c3, 0xdbdbdb, 0x9c9c9c, 0xf5f5f5) )
		page.add( MelodyneRadioBtn("Show tooltips", self.sibi, Pt(0,0), None, Pt(160, 210), None, 0xc1c1c1, 0x181818, None) )
		user_interface.add(page)

		pages.add(user_interface)
		
		self.sibi.add( pages)

gMelodyneDialogs = { "Correct Pitch": MelodyneCorrectPitch, "Export": MelodyneExport, "Preferences": MelodynePreferences, 
                     "Quantize Time": MelodyneQuantizeTime, "Note Leveling": MelodyneNoteLeveling,
