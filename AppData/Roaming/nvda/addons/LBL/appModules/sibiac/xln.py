...
class AD2(SIBINVDA):
	...
	def defineSIBI(self):
		...
		dlg.add( PushBtn("Set as default", sibi, Pt(884,580), Pt(884,580), 0x303541, 0xbdcae5,("silent_action",)) ) 
...
class AKPresetList(VList):
	...
	def onUp(self):
		box = Box(self.sibi.hwnd, 501, 533, 502, 534)
		box.leftClick()
		self.sibi.speakAfter(self.reactionTime())
		return True

	def onDown(self):
		box = Box(self.sibi.hwnd, 582, 533, 583, 534)
		box.leftClick()
		self.sibi.speakAfter(self.reactionTime())
		return True
...
class AK(SIBINVDA):
	sname = "Addictive keys"
	
	def defineSIBI(self):
		self.sibi = SIBI(self.windowHandle, 1100, 700, ("block_arrows"))

		self.sibi.add( OpenBtn("Preset", self.sibi, Pt(114, 28), Pt(300, 45), None, None, ("dynamic_text","silent_action")) )
		self.sibi.add( OpenBtn("Save preset", self.sibi, Pt(380, 47), Pt(380, 47), 0x000000, 0xa2acc1, ("slow_reaction",)) ) 

		######
		dlg = Dialog("Preset Browser", self.sibi, Pt(248, 105), 0x050917, 0xd4c2a7, set())
		self.sibi.addDialog(dlg)
		dlg.add( Clickable("Filter by name", self.sibi, Pt(524,161), Pt(621,177), ("dynamic_text", "silent_action")) )
		dlg.add( AKPresetList("Preset", self.sibi, Pt(264, 190), Pt(618, 226), Pt(264, 230), Pt(264, 513), 0x182438, 0x29354a, ("slow_reaction",)) )
		dlg.add( Clickable("Set as startup", self.sibi, Pt(844, 495), Pt(844, 495), ("silent_action",)) ) 
		dlg.add( OpenBtn("OK", self.sibi, Pt(460, 580), Pt(460, 580), None, None, ("OK",)) )
		dlg.add( OpenBtn("Cancel", self.sibi, Pt(576, 580), Pt(576, 580), None, None, ("CANCEL",)) )

		###
		dlg = Dialog("Save preset", self.sibi, Pt(549, 93), 0x03040d, 0xe0cfbb, ())
		self.sibi.addDialog(dlg)
		dlg.add( Clickable("Name", self.sibi, Pt(398, 161), Pt(617, 176), ("dynamic_text", "silent_action")) ) 
		dlg.add( OpenBtn("Save", self.sibi, Pt(349, 359), Pt(349, 359), 0x0e121b, 0x2c3948, ("OK",)) )
		dlg.add( OpenBtn("Cancel", self.sibi, Pt(673, 359), Pt(673, 359), None, None, ("CANCEL",)) )
